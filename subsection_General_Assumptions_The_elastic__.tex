...
\begin{equation}
\dot{\boldsymbol{\epsilon_p}}=\dot{\lambda} \frac{\partial G\left(\bar{\boldsymbol{\sigma}}\right)}{\partial \boldsymbol{\sigma}}
\label{eqn:const8b}
\end{equation}

Where $\dot{\epsilon_p}$ is the plastic strain rate, $\dot{\lambda}$ is referred to as the plastic consistency parameter, and $G\left(\bar{\boldsymbol{\sigma}}\right)$ is the flow potential function. In both models presented here, the flow potential function is taken from the Drucker-Prager model:
...
Where $\Psi$ is the dilation angle of the material, $\bar{\sigma}_{0}$ is the initial yield stress, while $p$ and $q$ stress invariants represent the mises
...
In addition, to the flow rule, the hardening rule is prescribed to govern the increase/decrease in yield stress as the plastic strain increases. The hardening rule
