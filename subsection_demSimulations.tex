...
First, triaxial compressive strength test with a compressive strain rate of $0.05\%s^{-1}$ for $100s$ to observe the post yield behaviour of the DEM simulations under a series of different confining stresses showing a very strong pressure dependent yield. These simulations were designed in order to capture the physics implemented in the Drucker-Prager plasticity model with ductile damage, including a pressure dependent yield criterion and a damage initiation function that is dependent on the triaxiality. This load case was used for the parameter estimation in order to obtain the optimal parameter set.
