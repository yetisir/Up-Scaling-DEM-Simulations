...
def get_input_string(filename, localdir, quotepath=True):
    ...
    if quotepath:
        ...
        quote_chr = ''
    return r'\input{' + quote_chr + os.path.join(localdir, filename).replace('\\', '/') + quote_chr + '}'
...
def build_authorea_latex(localdir, builddir, latex_exec, bibtex_exec, outname,
    ...
    bibloc = os.path.join(get_in_path(localdir, builddir, pathtype), 'bibliography', 'biblio').replace('\\', '/')
